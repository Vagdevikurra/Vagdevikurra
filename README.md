...
# ── Step A: TOTAL accts_cnt per RCIF (sum ALL segments) ──────────────────────
# Original pw1 has one row per customer+segment.
# accts_cnt per row = count of arrangements in that segment.
# Total = SUM across all segments per customer.
pw1_total_accts = (
    pw1
    .groupBy("RCIF_NUMBER")
    .agg(
        F.sum("accts_cnt").alias("total_accts_cnt"),
...
# ── Step B: Top-priority Business_Group + division per RCIF ──────────────────
pw1_top_bg = (
    pw1
...
# ── Step C: Join total counts back to top Business_Group row ─────────────────
pw1_per_rcif = (
    pw1_top_bg
    .join(pw1_total_accts, pw1_top_bg["pw_rcif"] == pw1_total_accts["RCIF_NUMBER"], "left")
    .select(
        "pw_rcif",
        "pw_ip_id",
        "Business_Group",
        "division",
        F.col("total_accts_cnt").alias("accts_cnt"),
...
# Total counts per ip_id across all segments
pw1_total_accts_ip = (
    pw1
    .groupBy("ip_id")
    .agg(
...
pw1_per_ip = (
    pw1_top_bg_ip
    .join(pw1_total_accts_ip, pw1_top_bg_ip["pw_ip_id"] == pw1_total_accts_ip["ip_id"], "left")
    .select(
        "pw_ip_id",
        "Business_Group",
        "division",
