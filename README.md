...
# ── 1) Row counts ────────────────────────────────────────────────────────────────
print("\n[1] Row counts:")
cust_total  = spark.sql(f"SELECT COUNT(*) AS c FROM {DEFAULT_DB}.wealth_insights_cust7").collect()[0]["c"]
wealth_rows = spark.sql(f"SELECT COUNT(*) AS c FROM {DEFAULT_DB}.wealth_insights_cust7 WHERE fact_type = 'WEALTH'").collect()[0]["c"]
digital_rows = spark.sql(f"SELECT COUNT(*) AS c FROM {DEFAULT_DB}.wealth_insights_cust7 WHERE fact_type = 'DIGITAL'").collect()[0]["c"]
acct_rows   = spark.sql(f"SELECT COUNT(*) AS c FROM {DEFAULT_DB}.wealth_insights_acct7").collect()[0]["c"]

print(f"  wealth_insights_cust7 total     : {cust_total:>12,}")
print(f"    fact_type = WEALTH           : {wealth_rows:>12,}")
print(f"    fact_type = DIGITAL          : {digital_rows:>12,}")
print(f"  wealth_insights_acct7           : {acct_rows:>12,}")
...
spark.sql(f"""
    ...
    FROM {DEFAULT_DB}.wealth_insights_cust7
    WHERE fact_type = 'WEALTH'
      AND business_date = (SELECT MAX(business_date) FROM {DEFAULT_DB}.wealth_insights_cust7 WHERE fact_type = 'WEALTH')
...
spark.sql(f"""
    ...
    FROM {DEFAULT_DB}.wealth_insights_acct7
    GROUP BY business_date ORDER BY business_date
""").show(truncate=False)
