...
print("max_login_date :", max_login_date)
...
add_rcifs = (
    ...
    .union(
        spark.table("dm_ib.transmit_digital_logins")
        .select(F.col("rcif_id").cast("string").alias("RCIF_NUMBER"))
    )
    .distinct()
...
# =============================================================================
# STEP 5: DIGITAL FLAGS — from transmit_digital_logins
#
# channel = "Online" → OLB
# channel = "Mobile" → Mobile
# 90-day active window relative to max_login_date
# active_90 guard: login <= snap AND datediff <= 90
# =============================================================================
...
dig_latest = (
    ...
    .agg(
        F.max(F.when(F.col("channel") == "Online", F.col("login_date"))).alias("lst_login_olb"),
        F.max(F.when(F.col("channel") == "Mobile", F.col("login_date"))).alias("lst_login_mob"),
        # enrolled = has any record for that channel
    ...
    .withColumn("snap_dt", F.lit(max_login_date).cast(T.DateType()))
    ...
    .withColumn("customer_banking_number", F.col("RCIF_NUMBER"))  # placeholder
    .drop("_olb_enr", "_mob_enr", "snap_dt")
)
