...
# DM_IB Hive database (Digital Banking)
DM_IB = "dm_ib"
DIGITAL_BANKING_MASTER = f"{DM_IB}.digital_banking_master"
...
def build_customer_dimension(spark, business_date):
    """
    Build a customer dimension combining:
      - dm_ib.digital_banking_master: digital enrollment, IBN, login activity
      - EIL.M_INVOLVED_PARTY_H: demographics (age, gender, birth date)
      - EIL.D_INVOLVED_PARTY_ADDRESS_H: geography (city, state, country)
    
    Key output columns for V1:
      RCIF_CUST_NBR, IBN, DIGITAL_CUSTOMER_CHECK, CUSTOMER_TYPE,
      ENROLL_DATE, ACTIVE_RELATIONSHIP, OLB/MOB login counts,
      CUSTOMER_AGE, AGE_BANDING, CUSTOMER_GENERATION, CUSTOMER_GENDER,
      CITY, STATE, COUNTRY
    """
    customer_dim = spark.sql(f"""
        WITH DIG_BANK AS (
            -- dm_ib.digital_banking_master: digital enrollment & login activity
            SELECT
                rcif_customer_nbr AS RCIF_CUST_NBR,
                ibn AS IBN,
                customer_type AS CUSTOMER_TYPE,
                enroll_date AS ENROLL_DATE,
                active_relationship AS ACTIVE_RELATIONSHIP,
                olb_last_login_date,
                mob_last_login_date,
                30d_olb_login_count,
                30d_mob_login_count,
                ...
                mtd_mob_login_count,
                ods_business_dt
            FROM {DIGITAL_BANKING_MASTER}
            WHERE ods_business_dt = '{business_date}'
        ),

        CUST_DEMO AS (
            -- EIL: demographics (age, gender, birth date)
            SELECT
        ...
        SELECT DISTINCT
            cd.RCIF_CUST_NBR,
            cd.CUST_INTERNET_BANKING_NBR,

            -- Digital customer check: has a record in digital_banking_master
            CASE
            ...
            db.olb_last_login_date,
            db.mob_last_login_date,
            db.30d_olb_login_count,
            db.30d_mob_login_count,
            db.mtd_olb_login_count,
            db.mtd_mob_login_count,

            -- Mobile vs Online active flags (derived)
            CASE
                WHEN db.30d_mob_login_count > 0 THEN 'Y' ELSE 'N'
            END AS MOBILE_ACTIVE_30D,
            CASE
                WHEN db.30d_olb_login_count > 0 THEN 'Y' ELSE 'N'
            END AS OLB_ACTIVE_30D,
            CASE
                WHEN db.30d_mob_login_count > 0 OR db.30d_olb_login_count > 0
                    THEN 'Y' ELSE 'N'
            END AS DIGITALLY_ACTIVE_30D,

            -- From EIL demographics
            cd.CUSTOMER_GENDER,
            cd.CUSTOMER_AGE,
            cd.OVER_40_CHECK,
            cd.AGE_BANDING,
            cd.CUSTOMER_GENERATION,
            cd.CITY,
            cd.STATE,
            cd.COUNTRY

        FROM CUST_DEMO cd
        LEFT JOIN DIG_BANK db ON
            cd.RCIF_CUST_NBR = db.RCIF_CUST_NBR
    """)
...
# Source: Customer dimension (now includes dm_ib.digital_banking_master 
#         with 30d login counts, active flags) + TSMT for session detail
# ---------------------------------------------------------------------------
def v1_req04_customer_age_groups(df_auth, df_device, customer_dim):
    """
    Count digitally enrolled customers vs active mobile app users 
    per age group.
    
    Now uses dm_ib.digital_banking_master fields directly:
      - DIGITAL_CUSTOMER_CHECK: enrolled or not
      - MOBILE_ACTIVE_30D / OLB_ACTIVE_30D / DIGITALLY_ACTIVE_30D: activity flags
      - 30d_mob_login_count / 30d_olb_login_count: login volumes
    
    NOTE: RCIF is parsed from TSMT_AUTHENTICATION_H (DATA column),
          NOT TSMT_TMX_H (which has no DATA column).
    """
    # ---- A) Direct from customer dimension (digital_banking_master) ----
    # This is the most reliable source for enrolled vs active
    age_group_from_dim = customer_dim.groupBy(
        "AGE_BANDING", "CUSTOMER_GENERATION", "DIGITAL_CUSTOMER_CHECK"
    ).agg(
        countDistinct("RCIF_CUST_NBR").alias("total_customers"),
        _sum(when(col("DIGITALLY_ACTIVE_30D") == "Y", 1).otherwise(0)).alias("digitally_active_30d"),
        _sum(when(col("MOBILE_ACTIVE_30D") == "Y", 1).otherwise(0)).alias("mobile_active_30d"),
        _sum(when(col("OLB_ACTIVE_30D") == "Y", 1).otherwise(0)).alias("olb_active_30d"),
        _sum(col("`30d_mob_login_count`")).alias("total_mob_logins_30d"),
        _sum(col("`30d_olb_login_count`")).alias("total_olb_logins_30d"),
        _sum(col("`mtd_mob_login_count`")).alias("total_mob_logins_mtd"),
        _sum(col("`mtd_olb_login_count`")).alias("total_olb_logins_mtd")
    )

    # ---- B) Supplement with TSMT session-level detail ----
    # Parse RCIF from AUTHENTICATION_H DATA column (TMX has no DATA column)
    auth_with_rcif = parse_tsmt_data(
        df_auth,
        {"rcif_cust_nbr": "$.RCIF_CUST_NBR"}
    ).filter(col("rcif_cust_nbr").isNotNull())

    # Active mobile users from device table (session-level)
    active_mobile = df_device.filter(
        upper(col("DEVICE_OS_TYPE")).isin("IOS", "ANDROID")
    ).select("SESSION_ID", "USER_ID").distinct()

    auth_customers = auth_with_rcif.select(
        col("rcif_cust_nbr").alias("RCIF_CUST_NBR"),
        col("SESSION_ID")
    ).distinct()

    # Session-based digitally active users by age group
    session_by_age = auth_customers.join(
        customer_dim, on="RCIF_CUST_NBR", how="inner"
    ).groupBy("AGE_BANDING", "CUSTOMER_GENERATION") \
     .agg(
         countDistinct("RCIF_CUST_NBR").alias("session_active_customers"),
         countDistinct("SESSION_ID").alias("total_sessions")
     )

    # Session-based mobile-active users by age group
    mobile_sessions = auth_customers.join(
    ...
    mobile_by_age = mobile_sessions.join(
     ...
     .agg(
         countDistinct("RCIF_CUST_NBR").alias("session_mobile_users")
     )

    # Combine dimension-based and session-based
    age_group_summary = age_group_from_dim.join(
        session_by_age, on=["AGE_BANDING", "CUSTOMER_GENERATION"], how="left"
    ).join(
        mobile_by_age, on=["AGE_BANDING", "CUSTOMER_GENERATION"], how="left"
    ).fillna(0, subset=["session_active_customers", "total_sessions", "session_mobile_users"])
...
def build_v1_dashboard(spark, business_date, start_date, end_date):
    ...
    results["req04_customer_age_groups"] = v1_req04_customer_age_groups(
        df_auth, df_device, customer_dim
    )
