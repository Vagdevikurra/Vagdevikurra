...
# ── RCIF bridge (birth_date NOT NULL, has address, latest snapshot) ───────────────
spark.sql(f"""
    CREATE OR REPLACE TEMP VIEW rcif_bridge AS
    SELECT DISTINCT
        CAST(ip.rcif_cust_nbr AS string)      AS rcif_number,
        ip.cust_internet_banking_nbr          AS ibn
    ...
    WHERE CAST(ip.business_date AS date) = date('{cust_dt}')
...
# ══════════════════════════════════════════════════════════════════════════════════
# DIGITAL FLAGS — LATEST DAY ONLY (matching Power BI's Digital table query)
# Power BI: WHERE ods_business_dt = (SELECT MAX(ods_business_dt) ...)
# This means flags are STATIC — same for every month
# Previous attempt failed because it used MAX across ALL months
# This version uses ONLY the latest day
# ══════════════════════════════════════════════════════════════════════════════════
spark.sql(f"""
    ...
    GROUP BY ibn
""")
print(f"[OK] dig_latest (single day: {max_dig_dt})")

# ── Static flags per RCIF through bridge ─────────────────────────────────────────
spark.sql("""
...
""")
print("[OK] rcif_dig_flags (static, latest day only)")
...
spark.sql(f"""
    ...
    -- WEALTH rows: STATIC flags from latest digital day
    SELECT
    ...
    -- DIGITAL rows (per-month for Top of Company measures)
    SELECT
...
# ── RCIF bridge (same as Step 1 — birth_date NOT NULL, has address) ──────────────
spark.sql(f"""
    CREATE OR REPLACE TEMP VIEW rcif_bridge AS
    SELECT DISTINCT
        CAST(ip.rcif_cust_nbr AS string) AS rcif_number,
