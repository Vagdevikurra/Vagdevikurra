...
# ── RCIF bridge (same as Step 1 — birth_date NOT NULL, latest snapshot) ──────────
spark.sql(f"""
...
# ══════════════════════════════════════════════════════════════════════════════════
# WRITE: Account table — single CREATE TABLE
# InvestPath filtered through rcif_bridge so only valid customers appear
# ip_accounts_cnt computed via GROUP BY + self-join (no window function)
# ══════════════════════════════════════════════════════════════════════════════════
print("\n[WRITE] Account table ...")
...
spark.sql(f"""
    ...
    WITH ip_raw AS (
        ...
        -- Filter through rcif_bridge: only customers with birth_date
