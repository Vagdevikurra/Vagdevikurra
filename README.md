...
# Keep pw1 rows as-is (one per customer+segment) — exact original SQL structure.
# Power BI DISTINCTCOUNT handles deduplication per Business_Group.
# =============================================================================

# ── rcif_dig filtered to valid RCIF universe ──────────────────────────────────
rcif_dig_filtered = (
    rcif_dig
...
# pw1 (multiple rows per customer per segment) LEFT JOIN rcif_dig on RCIF_NUMBER
# Preserves exact original row structure Power BI expects
wealth_insights_customer = (
    pw1
    .join(
        rcif_dig_filtered,
        pw1["RCIF_NUMBER"] == rcif_dig_filtered["rd_rcif"],
        "left"
    )
    .select(
        pw1["RCIF_NUMBER"],
...
# inv_df LEFT JOIN pw1 on ip_id (top Business_Group per ip for account context)
pw1_ip_top = (
    pw1
    .withColumn("bg_rank",
...
wealth_insights_account = (
    inv_df
    .join(pw1_ip_top, inv_df["ip_id"] == pw1_ip_top["pw_ip_id"], "left")
    .select(
