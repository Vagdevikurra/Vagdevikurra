...
spark.sql("""
    ...
    by_rcif AS (
        SELECT
            ...
            MAX(CASE WHEN birth_date IS NOT NULL THEN 1 ELSE 0 END) AS has_birth_date
...
# ── RCIF bridge: latest snapshot, birth_date NOT NULL ────────────────────────────
# This is the set of RCIFs eligible for digital flag matching (from old code sec 7)
spark.sql(f"""
...
""")
print("[OK] rcif_bridge (latest snapshot, birth_date NOT NULL)")
...
# ── Digital flags per month per RCIF — only through rcif_bridge ──────────────────
# Bridge on ibn, then aggregate to rcif_number + month (Power BI DAX style)
spark.sql("""
...
""")
print("[OK] digital_flags_monthly (bridge on ibn, agg per rcif per month)")
...
spark.sql(f"""
    ...
    -- WEALTH rows: per-month flags via bridge-filtered digital
    SELECT
        w.business_date,
        w.rcif_number,
    ...
    -- DIGITAL rows (full company digital population)
    SELECT
