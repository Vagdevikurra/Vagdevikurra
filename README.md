...
# ---------- latest snapshot ----------
snapshot_dt = (
    DBM.select(F.max(F.to_date("ods_business_dt")).alias("dt"))
       .first()["dt"]
)
...
rcif = (
    IP
    .where(F.to_date("business_date") ==
           IP.select(F.max(F.to_date("business_date"))).first()[0])
...
wid2 = (
    dig_customer
    .join(rcif, on="reltibn", how="inner")
    .withColumn(
        "mobile_active_flag",
